...
// USAGE
// Encode : fileName                -> Generates payload, disInfo, Huffman Shaped WT and seed
...
void Encode() {
	...
	int_vector<> lengths(fileLength * 8, 0);
	int index = 0;
	unsigned char numZeroes = 0;

	for(int i = 0; i < disInfoIndex; i++) {
        unsigned char val = disInfoData[val];
        for(int j = 7; j >= 0; j--) {
            if((val >> j) & 1 == 1) {
                lengths[index] = numZeroes + 1;
                numZeroes = 0;
                index++;
            } else {
                numZeroes++;
	...
	wt_huff_int<rrr_vector<63>> wtHUFF;
	construct_im(wtHUFF, lengths);

	printf("\r\nHUFF WT size in MB: %f", size_in_mega_bytes(wtHUFF));

	std::ofstream outHUFF("wt_huff_data.sdsl");
	wtHUFF.serialize(outHUFF);
